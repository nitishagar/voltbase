/**
 * voltbase Worker (S4): the thin remote surface.
 * - `GET /` — static landing fragment (contains "voltbase").
 * - `GET /healthz` — `{"ok":true,"stage":4}` with STAGE from src/lib/stage.ts.
  ...
  app.get('/', (c) =>
    c.html(
      '<!doctype html><html lang="en"><head><meta charset="utf-8"><title>voltbase</title></head>' +
        '<body><h1>voltbase</h1><p>open-core EV charging-data tooling (stage 4 API + MCP)</p></body></html>',
    ),
  );
