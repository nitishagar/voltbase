#!/usr/bin/env bash
# scripts/verify.sh — S7 gate chain (mirrors the lumen validate chain):
# typecheck && lint && build && bundle-budget && TTFB-budget && test && smoke && check:banned,
...
# TTFB note: scripts/check-ttfb.mjs is local-only via app.request (no network).
set -euo pipefail
